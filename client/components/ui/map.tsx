...
function createPinImageData(fill: string): ImageData {
  const dpr = 2;
  ...
  ctx.beginPath();
  ctx.ellipse(cx, PIN_H - 2, 5, 2, 0, 0, Math.PI * 2);
  ctx.fill();

  ctx.beginPath();
  ctx.moveTo(cx, PIN_H - 2);
  ctx.quadraticCurveTo(cx - r * 1.35, headY + r * 0.55, cx - r, headY - r * 0.15);
  ctx.arc(cx, headY, r, Math.PI, 0, false);
  ctx.quadraticCurveTo(cx + r * 1.35, headY + r * 0.55, cx, PIN_H - 2);
  ctx.closePath();
  ctx.fillStyle = fill;
  ctx.fill();
  ...
  ctx.beginPath();
  ctx.arc(cx - 2.5, headY - 2.5, 2.2, 0, Math.PI * 2);
  ctx.fill();

  return ctx.getImageData(0, 0, canvas.width, canvas.height);
}

function syncPinImages(
  map: MapLibreGL.Map,
  prefix: string,
  ...
  map.addImage(names[0], createPinImageData(clusterColors[0]), { pixelRatio: 2 });
  map.addImage(names[1], createPinImageData(clusterColors[1]), { pixelRatio: 2 });
  map.addImage(names[2], createPinImageData(clusterColors[2]), { pixelRatio: 2 });
  map.addImage(names[3], createPinImageData(pointColor), { pixelRatio: 2 });
}
...
> = {
  ...
  /** Circle clusters (default) or map-pin markers */
...
>({
  ...
  const pinPrefix = `cluster-pin-${id.replace(/:/g, "")}`;
  ...
  useEffect(() => {
    ...
    if (markerStyle === "pin") {
      syncPinImages(map, pinPrefix, clusterColors, pointColor);
    }
    ...
    if (markerStyle === "pin") {
      ...
      map.addLayer({
        ...
        layout: {
          ...
          "text-font": ["Open Sans Bold", "Open Sans"],
          "text-size": 11,
          "text-anchor": "center",
          "text-offset": [0, -1.55],
          "icon-allow-overlap": true,
      ...
      map.addLayer({
        ...
        filter: ["!", ["has", "point_count"]],
        layout: {
          "icon-image": `${pinPrefix}-pin-point`,
          "icon-size": 0.62,
          "icon-anchor": "bottom",
          "icon-allow-overlap": true,
          "icon-ignore-placement": true,
    ...
    return () => {
      try {
        ...
        if (map.getSource(sourceId)) map.removeSource(sourceId);
        if (markerStyle === "pin") {
          for (const suffix of ["pin-0", "pin-1", "pin-2", "pin-point"]) {
            const name = `${pinPrefix}-${suffix}`;
            if (map.hasImage(name)) map.removeImage(name);
          }
        }
      } catch {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isLoaded, map, sourceId, markerStyle]);
  ...
  useEffect(() => {
    ...
    if (markerStyle === "pin") {
      if (
        ...
        prev.markerStyle !== markerStyle
      ) {
        syncPinImages(map, pinPrefix, clusterColors, pointColor);
        if (map.getLayer(clusterLayerId)) {
          map.setLayoutProperty(clusterLayerId, "icon-image", [
            "step",
            ["get", "point_count"],
            `${pinPrefix}-pin-0`,
            clusterThresholds[0],
    ...
    } else if (map.getLayer(clusterLayerId) && colorsChanged) {
