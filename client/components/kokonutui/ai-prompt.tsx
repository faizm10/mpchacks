...
interface AIPromptProps {
  ...
  className?: string;
  disabled?: boolean;
  /** Sidebar / embedded chat — hides promo header, tighter sizing */
  compact?: boolean;
}
...
export default function AI_Prompt({
  ...
  return (
    <div className={cn(compact ? "w-full" : "w-4/6 py-4", className)}>
      <div
        className={cn(
          "overflow-hidden rounded-xl border border-black/10 bg-black/[0.04] dark:border-white/10 dark:bg-white/[0.04]",
          compact ? "p-0" : "rounded-2xl p-1.5 pt-4"
        )}
      >
        {!compact && (
        ...
        <div className="relative flex flex-col">
          <div className="overflow-y-auto" style={{ maxHeight: compact ? "120px" : "400px" }}>
            <Textarea
              className={cn(
                "w-full resize-none border-none bg-transparent px-3 py-2.5 text-sm placeholder:text-black/50 focus-visible:ring-0 focus-visible:ring-offset-0 dark:text-white dark:placeholder:text-white/50",
                compact ? "min-h-[44px] rounded-none" : "min-h-[72px] rounded-xl rounded-b-none bg-black/5 px-4 py-3 dark:bg-white/5"
              )}
              id="ai-input-15"
              onChange={(e) => {
          ...
          <div
            className={cn(
              "flex items-center justify-between gap-2 border-black/10 border-t bg-black/[0.03] px-2 dark:border-white/10 dark:bg-white/[0.03]",
              compact ? "h-10 py-1" : "relative h-14 bg-black/5 dark:bg-white/5"
            )}
          >
            {!compact ? (
              ...
              <>
                <DropdownMenu>
                  <DropdownMenuTrigger
                    render={
                      <Button
                        className="flex h-7 max-w-[140px] items-center gap-1 truncate rounded-md px-1.5 text-[11px] hover:bg-black/10 focus-visible:ring-1 focus-visible:ring-[#7F77DD] focus-visible:ring-offset-0 dark:text-white dark:hover:bg-white/10"
                        variant="ghost"
                      />
                    }
                  >
                    <AnimatePresence mode="wait">
                      ...
                      >
                        <span className="shrink-0 scale-75">{MODEL_ICONS[selectedModel]}</span>
                  ...
                  >
                    {models.map((model) => (
                      ...
                      >
                        ...
                        {selectedModel === model && (
                          <Check className="h-4 w-4 text-[#7F77DD]" />
                        )}
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    value.trim() && !disabled
                      ? "bg-[#7F77DD] text-white hover:bg-[#6B63C9]"
